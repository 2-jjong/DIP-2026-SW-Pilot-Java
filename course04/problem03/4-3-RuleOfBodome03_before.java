...
class AllPurposeDevice implements DeviceBefore {
...
class DeviceControllerBefore {
    public void controlDevice(DeviceBefore device) {
        device.powerOn();
        device.activateFeature();
    }

    public void powerOffAllDevices(DeviceBefore... devices) {
        for (DeviceBefore device : devices) {
            device.powerOff();
        }
    }
}
...
class BiodomeFamily09_Before {
    public static void main(String[] args) {
        DeviceBefore allPurposeDevice1 = new AllPurposeDevice("스마트 라이트", "자동 빛 색상 조절");
        DeviceControllerBefore controller = new DeviceControllerBefore();
        controller.controlDevice(allPurposeDevice1);

        DeviceBefore allPurposeDevice2 = new AllPurposeDevice("냉장고", null);
